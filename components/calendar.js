...
function Calendar() {
  ...
  function handleClick(day){
    selectDate(day);
    const listDate = new Date(selectedDate.getFullYear(), selectedDate.getMonth(), day);
    const url = listDate.toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' }).replace(/\//g, '-');
  ...
  return (
    ...
    <div className={`${styles['elegant-calencar']} d-md-flex`}>
      <div className={`${styles['wrap-header']} wrap-header d-flex align-items-center`}>
            ...
            {monthData.map((week, index) => (
              <tr key={index}>
                {week.map((day, dayIndex) => {
                  const date = new Date(selectedDate.getFullYear(), selectedDate.getMonth(), day).toLocaleDateString(
                    'en-GB'
                  );
                  console.log(date);
